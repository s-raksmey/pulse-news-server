  ...
  try {
    ...
    if (articlesWithNullCategory.length > 0) {
      ...
      articlesWithNullCategory.forEach(article => {
        console.log(`   - "${article.title}" (${article.slug}) - topic: ${article.topic || "none"}`);
      });
    } else {
    ...
    const articlesWithCategory = await prisma.article.count({ where: { categoryId: { not: null } } });
    ...
    sampleArticles.forEach(article => {
      ...
      console.log(`     category: ${article.category ? `${article.category.slug} (${article.category.name})` : "null"}`);
